  ...
  <v-app-bar color="primary" density="compact">
    <v-menu>
      <template v-slot:activator="{ props }">
        <v-btn variant="text" class="text-none" style="flex: 0 0 auto;" v-bind="props">
          <v-icon start>mdi-menu</v-icon>
          <span class="text-h6">Архив</span>
        </v-btn>
      </template>
      <v-list>
        <v-list-item title="Документы" to="/"></v-list-item>
        <v-list-item title="Наборы изменений" to="/branches"></v-list-item>
        <v-list-item title="Задачи" to="/tasks"></v-list-item>
        <v-list-item title="Пользователи" to="/users"></v-list-item>
      </v-list>
    </v-menu>
    ...
    <v-menu>
      ...
      <v-list>
        <template v-if="user">
          <v-list-subheader>{{ user.display_name || user.username }}</v-list-subheader>
          ...
          <v-list-item title="Выйти" @click="handleLogout"></v-list-item>
        </template>
        <template v-else>
          <v-list-item title="Войти" to="/login"></v-list-item>
        </template>
      </v-list>
    </v-menu>
